...
#ifdef __DARWIN__
	if (rw == RW_READER)
		atomic_inc_32(&rwlp->rw_readercnt);
	else
#endif
...
rw_exit(krwlock_t *rwlp)
{
	...
	ASSERT(rwlp->rw_owner != (void *)-1UL);
#ifdef __DARWIN__
	if (rwlp->rw_owner != thr_self()) {
		if (rwlp->rw_readercnt > 0)
			atomic_dec_32(&rwlp->rw_readercnt);
	} else
#endif
		rwlp->rw_owner = NULL;
...
#if 1
	if (ret != 0 && ret !=EINTR)
	(void) fprintf(stderr, "cv_wait: cv %p - err %d unexpected!\n", ret);
#endif
...
uint64_t
zfs_fuid_table_load(objset_t *os, uint64_t fuid_obj, avl_tree_t *idx_tree, avl_tree_t *domain_tree)
{
	return (0);
}
#endif
