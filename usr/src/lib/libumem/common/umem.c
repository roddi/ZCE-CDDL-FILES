...
#define	CPUHINT()		(uint32_t)thr_self()
...
#define	umem_log_init(size)	(umem_log_header_t *)NULL
#define	umem_log_event(lp, cp, sp, addr)	
...
#ifdef __DARWIN__
	if (P2PHASE((uintptr_t)slab, vmp->vm_quantum) != 0)
		printf("umem_slab_create: P2PHASE(%p, %d) = %d (slabsize %d, align %d) in %s\n", slab, (int)vmp->vm_quantum, (int)P2PHASE((uintptr_t)slab, vmp->vm_quantum), (int)slabsize, (int)cp->cache_align, vmp->vm_name);

	if (vmp != umem_memalign_arena)
		ASSERT(P2PHASE((uintptr_t)slab, cp->cache_align) == 0);
	else
#endif
...
umem_slab_alloc(umem_cache_t *cp, int umflag)
{
	...
	if (sp->slab_refcnt > sp->slab_chunks)
		printf("umem_slab_alloc: %s: slab_refcnt %d > slab_chunks %d\n", cp->cache_name, (int)sp->slab_refcnt, (int)sp->slab_chunks);
#endif
...
#ifdef __DARWIN__
void *
umem_cache_alloc(umem_cache_t *cp, int umflag)
{
	return _umem_cache_alloc(cp, umflag);
}
#else
...
#ifdef __DARWIN__
void
umem_cache_free(umem_cache_t *cp, void *buf)
{
	return _umem_cache_free(cp, buf);
}
#else
...
#error expecting UMEM_STANDALONE
static void
umem_st_update(void)
{
	ASSERT(MUTEX_HELD(&umem_update_lock));
	...
	int cflags)		/* cache creation flags */
{
	...
	if (umem_init_thr == thr_self())
		...
		if (umem_ready != UMEM_READY && umem_init() == 0) {
			errno = EAGAIN;
#if 1
			printf("umem_cache_create: not ready! (%d)\n", umem_ready);
#endif
			return (NULL);
		}
	}
	...
	if ((align & (align - 1)) != 0 || align > vmp->vm_quantum ||
		...
		errno = EINVAL;
#if 1
		printf("umem_cache_create: EINVAL!, align %d, quantum %d (%s)\n",
			(int)align, (int)vmp->vm_quantum, vmp->vm_name);
#endif
		return (NULL);
	}
	...
	if (cp == NULL) {
		errno = EAGAIN;
#if 1
		printf("umem_cache_create: EAGAIN!\n");
#endif
		return (NULL);
	}
	...
	if (vmp == umem_firewall_arena) {
		...
		for (chunks = 1; chunks <= UMEM_VOID_FRACTION; chunks++) {
			...
			if ((slabsize / chunks) < chunksize) {
				errno = ENOMEM;
#if 1
				printf("FAILED: overflow - slabsize %d, %d chunks, chunksize %d, quantum %d\n",
					slabsize, chunks, chunksize, vmp->vm_quantum);
#endif
				goto fail;
			}
...
#ifdef __DARWIN__
	memalign_arena = vmem_create("umem_memalign", NULL, 0, 4096,
	    heap_alloc, heap_free, heap_arena, 0, VM_NOSLEEP);
	if (memalign_arena == NULL)
		goto fail;
#else
