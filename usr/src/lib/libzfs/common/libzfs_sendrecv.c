...
#ifdef __DARWIN__
	/*
	 * No kernel KPI to get file offset, so grab it now
	 */
	fileoff = lseek(outfd, 0, SEEK_CUR);
	zc.zc_history_offset = (fileoff != -1) ? fileoff : 0;
#endif
...
#ifdef __DARWIN__
		/*
		 * Darwin only has uni-directional pipes
		 * so make sure polarity is correct!
		 */
		err = pipefd[0];
		pipefd[0] = pipefd[1];
		pipefd[1] = err;
