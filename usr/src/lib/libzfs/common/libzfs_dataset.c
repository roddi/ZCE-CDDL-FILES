...
#ifdef __DARWIN__
	if ((prop == ZFS_PROP_DEDUP) && (strcmp(propval, "off") != 0)) {
		zfs_error_aux(hdl, "data deduplication property is not "
		    "supported in this build");
		ret = zfs_error(hdl, EZFS_NOTSUP, errbuf);
		goto error;
	}
#endif
	if (prop == ZFS_PROP_VOLSIZE) {
		...
		if (str[0] == '/') {
...
#ifdef __DARWIN__
			/*
			 * undo effects of using altroot in previous betas...
			 */
			if (root[0] == '\0' && str[1] == '\0' &&
			    strncmp(source, zhp->zfs_name, strlen(source)) == 0) {
				source = "";
			...
			if (relpath[0] == '\0')
				(void) snprintf(propbuf, proplen, "%s%s",
				    root, str);
#ifdef __DARWIN__
			else if (root[0] == '\0' && str[0] == '\0' && relpath[0] != '@')
				(void) snprintf(propbuf, proplen, "/Volumes/%s", relpath);
#endif  /* __DARWIN__ */
			else
