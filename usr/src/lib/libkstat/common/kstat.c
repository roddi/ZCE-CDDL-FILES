...
#define	ioctl(fd, req, ksp)  \
	((ioctl(fd, req, ksp) == -1) ? -1 :  \
	 ((ksp)->ks_ioc_error && (errno = (ksp)->ks_ioc_error)) ? -1 : 0)
#endif /* __DARWIN__ */
...
kstat_chain_update(kstat_ctl_t *kc)
{
	...
	kid_t kcid;
#ifdef __DARWIN__
	if (ioctl(kc->kc_kd, KSTAT_IOC_CHAIN_ID, &k0) != 0)
		return (-1);
	kcid = k0.ks_chain_id;
#else
...
kstat_lookup(kstat_ctl_t *kc, char *ks_module, int ks_instance, char *ks_name)
{
	...
	for (ksp = kc->kc_chain; ksp; ksp = (kstat_t *)ksp->ks_next) {
		if ((ks_module == NULL ||
