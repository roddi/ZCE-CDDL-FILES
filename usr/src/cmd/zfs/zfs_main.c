...
get_usage(zfs_help_t idx)
{
	...
	case HELP_DESTROY:
#ifdef __DARWIN__
		return (gettext("\tdestroy [-rRf] <filesystem>\n"
#else
	...
	case HELP_RECEIVE:
#ifdef __DARWIN__
		return (gettext("\treceive [-vnFu] <filesystem|"
#else
	...
	case HELP_RENAME:
#ifdef __DARWIN__
		return (gettext("\trename <filesystem|snapshot> "
		    "<filesystem|snapshot>\n"
		    "\trename -p <filesystem> <filesystem>\n"
	...
	case HELP_HOLDS:
		return (gettext("\tholds [-H] [-r] <snapshot> ...\n"));
	case HELP_RELEASE:
...
zfs_do_set(int argc, char **argv)
{
	...
	*cb.cb_value = '\0';
	...
	ret = zfs_for_each(argc - 2, argv + 2, NIL,
...
#ifdef __DARWIN__
	if (strncmp(argv[0], "automatic", 10) == 0)
		ret = zfsx_auto_snapshot(mnttab_file, g_zfs, props, zfs_do_destroy);
	else
#endif
