...
#ifdef __DARWIN__
static int zpool_do_showdisks(int, char **);
#endif
...
#ifdef __DARWIN__
	{ NULL },
	{ "showdisks",	zpool_do_showdisks,	HELP_SHOWDISKS		},
#endif
};
...
print_prop_cb(int prop, void *cb)
{
	...
	if (zpool_prop_readonly(prop) || zpool_prop_setonce(prop))
		(void) fprintf(fp, "  NO   ");
	else
		(void) fprintf(fp, " YES   ");
...
#ifdef __DARWIN__
	if (!hasnorm) {
		if (add_prop_list("normalization", "formD", &fsprops, B_FALSE))
			goto errout;
	}
#endif
	if (dryrun) {
...
#ifdef __DARWIN__
	if (geteuid() != 0) {
		fprintf(stderr, "could not destroy '%s': permission denied\n",
		    zpool_get_name(zhp));
		exit(1);
	}	
#endif
...
#ifdef __DARWIN__
	if ((flags & ZFS_IMPORT_FROM_KERNEL) == 0)
		zpool_probe_devices(config);
#endif
...
zpool_do_import(int argc, char **argv)
{
	...
	char *endptr;
#ifdef __DARWIN__
	boolean_t altroot = B_FALSE;
#endif

	/* check options */
#ifdef __DARWIN__
	while ((c = getopt(argc, argv, ":ad:DEfFkmnNo:rR:T:VX")) != -1) {
#else
...
#ifdef __DARWIN__
	if ((argc == 1) && (strncmp(argv[0], "automatic", 10) == 0)) {
		return zfsx_auto_poolscrub();
	}
#endif
	if (argc < 1) {
		(void) fprintf(stderr, gettext("missing pool name argument\n"));
