...
make_leaf_vdev(nvlist_t *props, const char *arg, uint64_t is_log,
    boolean_t dryrun, boolean_t force, boolean_t isspare)
#else
...
#ifdef __DARWIN__
	if (type == VDEV_TYPE_DISK && !dryrun) {
		if (darwin_vdev_prep(path, MAXPATHLEN, force, isspare) != 0) {
			return (NULL);
		}
	}
#endif
...
construct_spec(nvlist_t *props, int argc, char **argv,
    boolean_t dryrun, boolean_t force)
#else
		...
		if ((type = is_grouping(argv[0], &mindev, &maxdev)) != NULL) {
			...
			for (c = 1; c < argc; c++) {
				...
				if ((nv = make_leaf_vdev(props, argv[c],
				    B_FALSE, dryrun, force, isspare)) == NULL)
#else
			 ...
			 */
#ifdef __DARWIN__
			wait_leaf_vdev(argv[0], is_log);
			if ((nv = make_leaf_vdev(props, argv[0], is_log,
			     dryrun, force, B_FALSE)) == NULL)
#else
    ...
    splitflags_t flags, int argc, char **argv)
{
	...
	if (argc > 0) {
#ifdef __DARWIN__
		if ((newroot = construct_spec(props, argc, argv, B_TRUE, B_FALSE))
		     == NULL) {
#else
    ...
    boolean_t replacing, boolean_t dryrun, int argc, char **argv)
{
	 ...
	 */
#ifdef __DARWIN__
	if ((newroot = construct_spec(props, argc, argv, dryrun, force)) == NULL)
#else
	if ((newroot = construct_spec(argc, argv)) == NULL)
#endif
		return (NULL);
