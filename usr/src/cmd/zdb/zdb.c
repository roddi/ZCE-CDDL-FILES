...
#ifdef __DARWIN__
	if (strncmp(dev, "/dev/disk", 9) == 0) {
		len++;
		path = malloc(len);
		(void) snprintf(path, len, "/dev/r%s", dev + 5);
	} else
#endif
...
main(int argc, char **argv)
{
	...
	if (tunable) {
		...
		if (argc != 1) {
			(void) fprintf(stderr, "Usage: zdb -tune [list | variable=value]\n");
			exit(1);
		}
		if (strcmp(argv[0], "list") == 0) {
			zfsx_list_tunables();
		} else {
			zfsx_set_tunables(argv[0]);
		}
		exit(0);
	}
#endif /* __DARWIN__ */
...
#ifdef __DARWIN__
	if (error && (errno == ENOENT) && ! dump_opt['e'] && !dump_opt['l']) {
		dump_opt['e']++;
		goto opt_e;
	}
#endif
