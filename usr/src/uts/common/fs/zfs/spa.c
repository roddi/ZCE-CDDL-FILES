...
#ifdef __DARWIN__
boolean_t	zio_taskq_sysdc = B_FALSE;
#else
...
#if defined(_KERNEL) && !defined(__DARWIN__)
static void
spa_thread(void *arg)
{
	callb_cpr_t cprinfo;
...
spa_unload(spa_t *spa)
{
	...
	if (spa->spa_sync_on) {
#ifdef __DARWIN__
		if (spa->spa_vdev_disappeared)
			txg_sync_abort(spa->spa_dsl_pool);
		else
#endif
...
#ifdef __DARWIN__
	ZFSX_NOTIFY_CREATE_POOL(spa_name(spa), spa_guid(spa));
#endif
	return (0);
}
...
#ifdef __DARWIN__
	ZFSX_NOTIFY_IMPORT_POOL(spa_name(spa), spa_guid(spa));
#endif
	return (0);
}
    ...
    boolean_t force, boolean_t hardforce)
{
	...
	if (new_state == POOL_STATE_DESTROYED)
		ZFSX_NOTIFY_DESTROY_POOL(spa_name(spa), spa_guid(spa));
	else
		ZFSX_NOTIFY_EXPORT_POOL(spa_name(spa), spa_guid(spa));
#endif
