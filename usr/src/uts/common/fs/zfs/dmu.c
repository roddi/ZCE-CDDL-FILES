...
#define	xuio_stat_init()
#define	xuio_stat_fini()
...
dmu_read_uio(objset_t *os, uint64_t object, uio_t *uio, uint64_t size)
{
	 ...
	 * to be reading in parallel.
	 */
	err = dmu_buf_hold_array(os, object,
#ifdef __DARWIN__
	    uio_offset(uio), size, TRUE, FTAG, &numbufs, &dbp);
#else
	    uio->uio_loffset, size, TRUE, FTAG, &numbufs, &dbp);
#endif
