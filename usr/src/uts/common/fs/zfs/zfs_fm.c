...
#ifdef __DARWIN__
	if ((vd != NULL) && (vd->vdev_path != NULL) && (spa != NULL) &&
	    strcmp(subclass, FM_EREPORT_ZFS_PROBE_FAILURE) == 0) {
		ZFSX_NOTIFY_VDEV_FAILURE(spa_name(spa), vd->vdev_path,
		    subclass, 0);
	}
#endif
...
#ifdef __DARWIN__
		if ((vd != NULL) && (vd->vdev_path != NULL) && (spa != NULL) &&
		    (strcmp(subclass, FM_EREPORT_ZFS_IO_FAILURE) == 0 ||
		     strcmp(subclass, FM_EREPORT_ZFS_IO) == 0)) {
			ZFSX_NOTIFY_DISK_ERROR(spa_name(spa), vd->vdev_path,
			    zio->io_offset, zio->io_error);
		}	
#endif
		/*
...
#ifdef __DARWIN__
		if ((vd->vdev_path != NULL) && (spa != NULL)) {
			ZFSX_NOTIFY_VDEV_FAILURE(spa_name(spa), vd->vdev_path,
			    subclass, stateoroffset);
		}	
#endif
		/*
...
#ifdef __DARWIN__
	if ((vd != NULL) && (vd->vdev_path != NULL) && (spa != NULL))
		ZFSX_NOTIFY_DISK_ERROR(spa_name(spa), vd->vdev_path, offset, ECKSUM);
#endif
...
#ifdef __DARWIN__
	if (info != NULL && (vd != NULL) && (vd->vdev_path != NULL) && (spa != NULL))
		ZFSX_NOTIFY_DISK_ERROR(spa_name(spa), vd->vdev_path, offset, ECKSUM);
#endif
	fm_nvlist_destroy(ereport, FM_NVA_FREE);
	fm_nvlist_destroy(detector, FM_NVA_FREE);
