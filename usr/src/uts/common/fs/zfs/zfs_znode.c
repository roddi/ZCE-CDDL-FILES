...
/*
 * Darwin doesn't support znode moves yet...
 */
#ifndef __DARWIN__
...
#ifdef __DARWIN__
    dmu_object_type_t obj_type, sa_handle_t *hdl, enum znode_alloc variant,
    boolean_t hasname)
#else
...
#endif
{
...
#ifdef __DARWIN__
	if ((zp = tsd_get(zfs_prealloc_key)) == NULL) {
	...
	if (sa_bulk_lookup(zp->z_sa_hdl, bulk, count) != 0 || zp->z_gen == 0) {
		if (hdl == NULL)
			sa_handle_destroy(zp->z_sa_hdl);
#ifdef __DARWIN__
		ASSERT(tsd_get(zfs_prealloc_key) == NULL);
#endif
    ...
    uint_t flag, znode_t **zpp, zfs_acl_ids_t *acl_ids)
{
	...
	if (!(flag & IS_ROOT_NODE)) {
#ifdef __DARWIN__
		while ((*zpp = zfs_znode_alloc(zfsvfs, db, 0, obj_type, sa_hdl,
		    ZA_RETAIN_PREDECESSOR, B_FALSE)) == NULL);
#else
...
int
#ifdef __DARWIN__
_zfs_zget(zfsvfs_t *zfsvfs, uint64_t obj_num, znode_t **zpp,
    enum znode_alloc variant)
#else
	...
	*zpp = NULL;
	...
	if (hdl != NULL) {
		...
		if (zp->z_unlinked) {
			...
			if ((holderr = zfsx_vnode_hold(zp, ZTOV(zp))) != 0) {
				struct timespec ts;

				/* NOTE: z_lock and OBJ ID locks were dropped */
				sa_buf_rele(db, NULL);

				/*
				 * We couldn't obtain a reference for this vnode
				 * because another thread is reclaiming it. So
				 * we return an error and zfs_get_data() will
				 * ignore it. The thread performing the reclaim
				 * is now responsible for the data.
				 */
				if ((variant == ZA_GETDATA) &&
				    (holderr == EALREADY)) {
					return (EALREADY);
				}

				if (retries > 1000) {
					printf("gave up on vnode ref for znode "
					       "%lld @ %p, vp %p\n", zp->z_id,
					       zp, zp->z_vnode);
					zp->z_vnode = NULL;
					zp->z_vid = 0;
				...
				ts.tv_sec = 0;
				ts.tv_nsec = 250000000;  /* 0.25 sec */
				msleep((caddr_t)&zp->z_vid, NULL, PVFS,
				    "zfs_zget", &ts);

				retries++;
				if (retries % 12 == 0) {
					printf("still waiting on znode %lld (%p) %s %s\n", zp->z_id, zp,
					S_ISREG(zp->z_mode) ? "REG" : S_ISDIR(zp->z_mode) ? "DIR" : S_ISLNK(zp->z_mode) ? "LNK" : "???",
					(holderr == ESTALE) ? "VTERM" : (holderr == EALREADY) ? "ZIL" : "");
				}
				goto top;
			}
#else
...
#ifdef __DARWIN__
	    doi.doi_bonus_type, NULL, variant, B_TRUE);
#else
...
zfs_rezget(znode_t *zp)
{
	...
	SA_ADD_BULK_ATTR(bulk, count, SA_ZPL_PARENT(zfsvfs), NULL,
	    &zp->z_parent, sizeof (zp->z_parent));
#endif
...
zfs_extend(znode_t *zp, uint64_t end)
{
	...
	int error;
#ifdef __DARWIN__
	uint64_t max_blksz = ZFSVFS_MAX_BLKSZ(zp);
#endif
	/*
...
#ifndef __DARWIN__
	/*
	 * In Darwin, this was already performed by earlier ubc_setsize()
	 */
