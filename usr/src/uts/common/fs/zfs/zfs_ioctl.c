 ...
 */
/*
 * Portions (c) copyright 2012, GreenBytes, Inc.  All rights reserved. 
 */
...
history_str_get(zfs_cmd_t *zc)
{
	...
	size_t namelen;
#endif
	if (zc->zc_history == NIL)
		return (NULL);

	buf = kmem_alloc(HIS_MAX_RECORD_LEN, KM_SLEEP);
#ifdef __DARWIN__
	if (copyinstr(zc->zc_history, buf, HIS_MAX_RECORD_LEN, &namelen) != 0) {
#else
...
zfs_secpolicy_read(zfs_cmd_t *zc, cred_t *cr)
{
	...
	printf("zfs_secpolicy_read: %s %d || %d\n", zc->zc_name ? zc->zc_name : "",
		INGLOBALZONE(curproc), zone_dataset_visible(zc->zc_name, NULL));
	return (ENOENT);
}
...
zfs_secpolicy_write_perms(const char *name, const char *perm, cred_t *cr)
{
	...
	if (error)
		printf("zfs_secpolicy_write_perms: %s error %d\n", name ? name : "", error);
	return (0);
}
    ...
    const char *perm, cred_t *cr)
{
	...
	if (error)
		printf("zfs_secpolicy_write_perms_ds: %s error %d\n", name ? name : "", error);
	return (error);
}
...
#ifdef __DARWIN__
	/*
	 * may also need to test zc->zc_name
	 */
	if (strncmp(vfs_statfs(vnode_mount(vp))->f_fstypename, MNTTYPE_ZFS,
	    MFSTYPENAMELEN) != 0) {
		VN_RELE(vp);
		return (EPERM);
	}
...
zfs_ioc_pool_create(zfs_cmd_t *zc)
{
	...
	if ((error = zfsx_config_props_valid(zc, config, props))) {
		nvlist_free(config);
		nvlist_free(props);
		return (error);
	}
...
#ifdef __DARWIN__
	if (error == 0 && zc->zc_cookie) {
		nvlist_t *spa_config = NULL;
		int alt_err;

		alt_err = spa_get_stats(zc->zc_name, &spa_config, NULL, 0);

		if ((alt_err == 0) && (spa_config != NULL)) {
			if ((alt_err = spa_export(zc->zc_name, NULL, B_TRUE,
			    B_TRUE))) {
				printf("zfs_ioc_pool_create: err %d during "
				    "kickstart (export)\n", alt_err);
			} else if ((alt_err = spa_import(zc->zc_name, spa_config, props,
			     ZFS_IMPORT_NORMAL))) {
				printf("zfs_ioc_pool_create: err %d during "
				    "kickstart (import)\n", alt_err);
			}
			...
			printf("zfs_ioc_pool_create: err %d during kickstart "
			    "(spa_open)\n", alt_err);
		}
	}
#endif
...
#ifdef __DARWIN__
	if (error == 0)
		(void) init_root_proxy(zc->zc_name);
#endif
	return (error);
}
...
zfs_ioc_set_prop(zfs_cmd_t *zc)
{
	...
	if (zc->zc_nvlist_dst != NIL && errors != NULL) {
...
zfs_ioc_pool_get_props(zfs_cmd_t *zc)
{
	...
	if (error == 0 && zc->zc_nvlist_dst != NIL)
...
zfs_ioc_recv(zfs_cmd_t *zc)
{
	...
	*tosnap++ = '\0';
	...
	if (zc->zc_history_offset)
		fp->f_offset = zc->zc_history_offset;
#endif
	off = fp->f_offset;
...
extern int find_proxy_device(const char *, uint64_t, uint64_t, uint64_t, char *, int);
extern uint64_t dataset_unique_guid(objset_t *);
 ...
 *
 * outputs:
 * zc_string	name of proxy device
 */
static int
zfs_ioc_retain_proxy(zfs_cmd_t *zc)
{
	uint64_t size = 0;
	...
	/*
	 * for the top-level dataset, use pool's size
	 */
	if (strchr(zc->zc_name, '/') == NULL) {
		...
		if ((error = spa_open(zc->zc_name, &spa, FTAG)) == 0) {
			...
			if ((error = spa_prop_get(spa, &nvl)) == 0) {
				nvlist_t *propval = NULL;

				if ((error = nvlist_lookup_nvlist(nvl, zpool_prop_to_name(ZPOOL_PROP_SIZE), &propval)) == 0) {
					if ((error = nvlist_lookup_uint64(propval, ZPROP_VALUE, &size))) {
						printf("zfs_ioc_retain_proxy: nvlist_lookup_uint64 failed '%s' (err %d)\n", zc->zc_name, error);
					}
					nvlist_free(propval);
				} else {
					printf("zfs_ioc_retain_proxy: nvlist_lookup_nvlist failed '%s' (err %d)\n", zc->zc_name, error);
				}
				nvlist_free(nvl);
			} else {
				printf("zfs_ioc_retain_proxy: spa_prop_get failed '%s' (err %d)\n", zc->zc_name, error);
			}
			spa_close(spa, FTAG);
		} else {
			printf("zfs_ioc_retain_proxy: can't open spa for '%s' (err %d)\n", zc->zc_name, error);
		}
	} else /* use dataset's size */ {
		objset_t *os;
		uint64_t refdbytes = 0, availbytes = 0, usedobjs = 0, availobjs = 0;

		if ((error = dmu_objset_hold(zc->zc_name, FTAG, &os)))
			return (error);

		dmu_objset_space(os, &refdbytes, &availbytes, &usedobjs, &availobjs);
		size = refdbytes + availbytes;
		pool_guid = spa_guid(dmu_objset_spa(os));
		ds_guid = dataset_unique_guid(os);

		dmu_objset_rele(os, FTAG);
	}

	if (find_proxy_device(zc->zc_name, size, pool_guid, ds_guid,
	    zc->zc_string, sizeof (zc->zc_string)) == 0)
		error = 0;
	else
		error = ENODEV;
 ...
 */
static int
zfs_ioc_release_proxy(zfs_cmd_t *zc)
{
	printf("zfs_ioc_release_proxy: '%s', '%s'\n", zc->zc_name, zc->zc_string);
	return (ENODEV);
}
#endif
...
static zfs_ioc_vec_t zfs_ioc_vec[] = {
	...
	{ zfs_ioc_obj_to_stats, zfs_secpolicy_diff, DATASET_NAME, B_FALSE,
	    POOL_CHECK_SUSPENDED },
#ifdef __DARWIN__
	{ zfs_ioc_retain_proxy, zfs_secpolicy_none, DATASET_NAME, B_FALSE,
	    POOL_CHECK_NONE },
	{ zfs_ioc_release_proxy, zfs_secpolicy_none, DATASET_NAME, B_FALSE,
	    POOL_CHECK_NONE },
#endif
};
...
zfsdev_open(dev_t dev, int flags, int devtype, struct proc *p)
{
	...
	/* Allocate soft state if requested. */
	if (flags & FEXCL) {
		minor_t minor = getminor(dev);
...
static int
zfsdev_close(dev_t dev, int flags, int devtype, struct proc *p)
{
	minor_t minor = getminor(dev);
...
static int
zfsdev_ioctl(dev_t dev, u_long cmd, caddr_t data, int flag, struct proc *p)
{
	zfs_cmd_t *zc;
...
//		return (zvol_ioctl(dev, cmd, arg, flag, cr, rvalp));
	...
	/* Make sure the command is ours */
	if (((char)IOCGROUP(cmd) != 'Z') ||
	    (IOCPARM_LEN(cmd) != sizeof (zfs_cmd_t))) {
		printf("zfsdev_ioctl: invalid command 0x%08x\n", (__uint32_t)cmd);
		return (EINVAL);
	}
	vec = ZFS_IOC_CMD(cmd);
	zc = (zfs_cmd_t *)data;

	/* Make sure the command is valid (in range) */
	if (vec >= sizeof (zfs_ioc_vec) / sizeof (zfs_ioc_vec_t)) {
		zc->zc_ioc_error = EINVAL;
		printf("zfsdev_ioctl: invalid selector %d (out of range)\n", vec);
		return (EINVAL);
	}

	if ((error = zfsx_secpolicy_config(vec, zc, ctx)))
		return (error);

	error = zfs_ioc_vec[vec].zvec_secpolicy(zc, (cred_t *)vfs_context_ucred(ctx));

	if (error)
		printf("zfsdev_ioctl: secpolicy error %d on command %d\n", error, vec);
	...
	if (error &&
	    ...
	    (error != EAGAIN) &&
	    (error != ESTALE))
		printf("zfsdev_ioctl: function error %d on command %d\n", error, vec);

	if ((error == 0) && (zfs_ioc_vec[vec].zvec_his_log == B_TRUE)) {
		/* skip logging kernel imports */
		if ((zfs_ioc_vec[vec].zvec_func != zfs_ioc_pool_import) ||
		    ((zc->zc_cookie & ZFS_IMPORT_FROM_KERNEL) == 0))
			zfs_log_history(zc);
	}
	/* 
...
/*
 * On Darwin we must use a cloning device since the request to
 * clone (an overloaded FEXCL flag) happens after the device
 * dev_t is already established.
 */
static int
zfsdev_clone(dev_t dev, int action)
{
	minor_t minor = getminor(dev);
...
/*
 * Global keys for our TSD
 */
uint_t zfs_fsyncer_key;
extern uint_t rrw_tsd_key;
extern uint_t zfs_active_key;
extern uint_t zfs_prealloc_key;

void
zfsx_ioctl_init(void)
{
	dev_t dev;
	...
	/*
	 * On Darwin we must use a cloning device since the request
	 * to clone (overloaded FEXCL flag) happens after the device
	 * dev_t is already established.
	 *
	 * TBD - should we have 2 dev nodes? A cloning and non-cloning?
	 */
	dev = zfs_major << 24;
	zfs_devnode = devfs_make_node_clone(dev, DEVFS_CHAR, UID_ROOT, GID_WHEEL,
	    0666, zfsdev_clone, MNTTYPE_ZFS, 0);

	tsd_create(&zfs_fsyncer_key, NULL);
	tsd_create(&rrw_tsd_key, NULL);
	tsd_create(&zfs_active_key, NULL);
	tsd_create(&zfs_prealloc_key, NULL);
}

void
zfsx_ioctl_fini(void)
{
	tsd_destroy(&zfs_prealloc_key);
	tsd_destroy(&zfs_active_key);
