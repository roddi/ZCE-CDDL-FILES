...
zfs_get_data(void *arg, lr_write_t *lr, char *buf, zio_t *zio)
{
	 ...
	 */
#ifdef __DARWIN__
	if (ZFS_ZGET_NOWAIT(zfsvfs, object, &zp) != 0)
		return (ENOENT);
...
#ifndef __DARWIN__
			/*
			 * Incorrect assertion since zfs_get_done() dropped
			 * the vnode reference the znode could have already
			 * been recycled!
			 */
...
top:
	*vpp = NULL;
	...
	if (zp == NULL) {
		 ...
		 */
		if ((ZP_GET_VTYPE(zp) == VDIR) && (mode & S_IWRITE)) {
			error = EISDIR;
			goto out;
		}
		/*
...
#ifdef __DARWIN__
	if ((flags & FNODELETEBUSY) && vnode_isinuse(vp, 0)) {
		error = EBUSY;
		goto out;
	}
#endif /* __DARWIN__ */
    ...
    caller_context_t *ct, int flags)
{
	 ...
	 */
#ifdef __DARWIN__
	uio_getiov(uio, 0, NULL, &bytes_wanted);
	if (uio_isuserspace(uio) || uio_iovcnt(uio) != 1) {
		bufsize = bytes_wanted;
		outbuf = kmem_alloc_large(bufsize, KM_SLEEP);
		odp = (dirent64_t *)outbuf;
	} else {
		bufsize = bytes_wanted;
		outbuf = NULL;
		odp = (dirent64_t *)CAST_DOWN(caddr_t, uio_curriovbase(uio));
	}
#else
	iovp = uio->uio_iov;
	...
	while (outcount < bytes_wanted) {
		...
		dtype = DT_DIR;  /* setup for special case below */
#endif
		/*
		...
		if (offset == 0) {
			...
			zap.za_normalization_conflict = 0;
#ifdef __DARWIN__
			objnum = ZFS_TO_DARWIN_ID(zfsvfs, parent);
#else
			objnum = parent;
#endif
...
#ifdef __DARWIN__
		} else if (offset == 2 && zfs_has_snapicon(zp)) {
			(void) strcpy(zap.za_name, ZFS_SNAPICON_NAME);
			zap.za_normalization_conflict = 0;
			objnum = ZFSCTL_INO_SNAPICON;
			dtype = DT_REG;
#endif
		} else {
			/*
		...
		if (flags & V_RDDIR_ENTFLAGS) {
...
#ifdef __DARWIN__
			eodp->ed_type = dtype;
			eodp->ed_namlen = strlen(zap.za_name);
			...
			/* In Darwin, offset only valid if extended */
#else
	...
	skip_entry:
		...
		if (offset > 2 || (offset == 2 &&
		    !(zfs_show_ctldir(zp) || zfs_has_snapicon(zp)))) {
#else
...
#ifdef __DARWIN__
	if (outbuf == NULL) {
		uio_update(uio, outcount);
	} else if ((error = uio_move(outbuf, (long)outcount, UIO_READ, uio))) {
		/*
		 * Reset the pointer.
		 */
		offset = uio_offset(uio);
...
update:
...
#ifdef __DARWIN__
	if (outbuf != NULL)
		kmem_free_large(outbuf, bufsize);
#else
	if (uio->uio_segflg != UIO_SYSSPACE || uio->uio_iovcnt != 1)
	...
	/* pass entcount return value into unused ct arg */
	if (ct)
		*((int *)ct) = entcount;
#else
...
#ifdef __DARWIN__
	if ((vnode_mount(tdvp) != vnode_mount(sdvp)) || zfsctl_is_node(tdvp)) {
#else
...
#ifdef __DARWIN__
	vnode_t 	**vpp = (vnode_t **)ct;  /* Darwin needs the vnode */
#endif
...
#ifdef __DARWIN__
	*vpp = ZTOV(zp);  /* Darwin KPI needs the vnode */
#else
	VN_RELE(ZTOV(zp));
#endif
	if (zfsvfs->z_os->os_sync == ZFS_SYNC_ALWAYS)
    ...
    caller_context_t *ct, int flags)
{
	 ...
	 */
	if (vnode_vtype(svp) == VDIR) {
		ZFS_EXIT(zfsvfs);
		return (EPERM);
	}

#ifdef __DARWIN__
	if ((vnode_mount(svp) != vnode_mount(tdvp)) || zfsctl_is_node(svp)) {
#else
...
/*
 * Wrappers to Solaris ZPL VOPs that are needed for Darwin
 */

int
_zfs_lookup(vnode_t *dvp, char *nm, vnode_t **vpp, int flags, cred_t *cr)
{
	return zfs_lookup(dvp, nm, vpp, NULL, flags, NULL, cr, NULL, NULL, NULL);
}

int
_zfs_create(vnode_t *dvp, char *name, vattr_t *vap, vcexcl_t excl,
    vnode_t **vpp, cred_t *cr, int flag, vsecattr_t *vsecp)
{
	return zfs_create(dvp, name, vap, excl, 0, vpp, cr, flag, NULL, vsecp);
}

int
_zfs_remove(vnode_t *dvp, char *name, int flags)
{
	return zfs_remove(dvp, name, NULL, NULL, flags);
}

int
_zfs_mkdir(vnode_t *dvp, char *dirname, vattr_t *vap, vnode_t **vpp,
    cred_t *cr, int flags, vsecattr_t *vsecp)
{
	return zfs_mkdir(dvp, dirname, vap, vpp, cr, NULL, flags, vsecp);
}

int
_zfs_rmdir(vnode_t *dvp, char *name, int flags)
{
	return zfs_rmdir(dvp, name, NULL, NULL, NULL, flags);
}

int
_zfs_readdir(vnode_t *vp, uio_t *uio, int *eofp, int *numdirent, int flags)
{
	return zfs_readdir(vp, uio, NULL, eofp, (caller_context_t *)numdirent, flags);
}

int
_zfs_rename(vnode_t *sdvp, char *snm, vnode_t *tdvp, char *tnm, int flags)
{
	return zfs_rename(sdvp, snm, tdvp, tnm, NULL, NULL, flags);
}

int
_zfs_symlink(vnode_t *dvp, char *name, vattr_t *vap, char *link, cred_t *cr,
    vnode_t **vpp, int flags)
{
	return zfs_symlink(dvp, name, vap, link, cr, (caller_context_t *)vpp, flags);
}

int
_zfs_readlink(vnode_t *vp, uio_t *uio)
{
	return zfs_readlink(vp, uio, NULL, NULL);
}

int
_zfs_link(vnode_t *tdvp, vnode_t *svp, char *name, cred_t *cr, int flags)
{
	return zfs_link(tdvp, svp, name, cr, NULL, flags);
}

int
_zfs_space(vnode_t *vp, int cmd, flock64_t *bfp, int flag, offset_t offset, cred_t *cr)
{
	return zfs_space(vp, cmd, bfp, flag, offset, cr, NULL);
}
#endif /* !__DARWIN__ */
