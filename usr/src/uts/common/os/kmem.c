...
#define	debug_enter(msg)  \
	 PE_enter_debugger(msg)
#endif /* __DARWIN__ */
...
#ifdef __DARWIN__
static list_t		kmem_deferred_slab_list;
static size_t		kmem_deferred_slab_count;
static kmutex_t		kmem_deferred_slab_lock;
...
#define	kmem_log_event(lp, cp, sp, addr)
...
#ifdef __DARWIN__
static void
kmem_recycle_timeout(void *arg)
{
	deferred_slab_t *tsp = (deferred_slab_t *)arg;
	deferred_slab_t *dsp = NULL;

	mutex_enter(&kmem_deferred_slab_lock);

	if (kmem_deferred_slab_count > 0) {
		for (dsp = list_head(&kmem_deferred_slab_list); dsp != NULL;
		    dsp = list_next(&kmem_deferred_slab_list, dsp)) {
			if (dsp != tsp)
				continue;

			list_remove(&kmem_deferred_slab_list, dsp);
			kmem_deferred_slab_count--;
			break;
		}
	}

	mutex_exit(&kmem_deferred_slab_lock);

	if (dsp == tsp)
		vmem_free(tsp->slab_vmp, tsp, KMEM_VMEM_ALLOC_SLABSIZE);
}
#endif /* __DARWIN__ */
...
#ifdef __DARWIN__
	ASSERT((vmp == kmem_default_arena) || (vmp == kmem_msb_arena));
	slab = NULL;

	/*
	 * Check our deferred list before going to the virtual memory subsystem.
	 */
	if (slabsize == KMEM_VMEM_ALLOC_SLABSIZE) {
		deferred_slab_t *dsp = NULL;

		mutex_enter(&kmem_deferred_slab_lock);
		if ((kmem_deferred_slab_count > 0) &&
		    (dsp = list_remove_head(&kmem_deferred_slab_list)) != NULL) {
			kmem_deferred_slab_count--;
			slab = (char *)dsp;
		}
		mutex_exit(&kmem_deferred_slab_lock);
		if (dsp != NULL) {
			/*
			 * IMPORTANT - take ourself out of the thread
			 * call queue to avoid  duplicates in the queue!
			 *
			 * The thread call queue is expecting unique entries
			 */
			untimeout(kmem_recycle_timeout, dsp, 0); 
			goto gotslab;
		}
	}
#endif
...
#ifdef __DARWIN__
	/*
	 * Defer releasing vmem slabs to the virtual memory subsystem
	 * for a brief period to avoid rapid cycling of these slabs.
	 * kmem_slab_create() will always check our deferred list before
	 * going to the virtual memory subsystem.
	 *
	 * Keep a maximum of 25 on the list.
	 */

	if (cp->cache_slabsize == KMEM_VMEM_ALLOC_SLABSIZE) {
		deferred_slab_t *dsp = slab;
		boolean_t deferred;

		mutex_enter(&kmem_deferred_slab_lock);

		if (kmem_deferred_slab_count < KMEM_VMEM_MAX_DEFER_COUNT) {
			dsp->slab_ttl = 0;
			dsp->slab_vmp = vmp;
			list_insert_tail(&kmem_deferred_slab_list, dsp);
			kmem_deferred_slab_count++;
			deferred = B_TRUE;
		} else {
			deferred = B_FALSE;
		 ...
		 */
		if (deferred)
			(void) timeout(kmem_recycle_timeout, dsp, KMEM_VMEM_DEFER_DELAY);
		else
...
/*
 * Reap some memory from a specific cache
 */
void
	...
	int cflags)		/* cache creation flags */
{
...
#ifdef __DARWIN__
		if (cflags & KMC_VMEM_ALLOC)
			bestfit = KMEM_VMEM_ALLOC_SLABSIZE;
		if (cflags & KMC_ZIO_BUF)
			bestfit = VMEM_ZIO_BUF_SLABSIZE(bufsize);
#endif
	...
	if ((cp->cache_kstat = kstat_create("unix", 0, cp->cache_name,
		...
		cp->cache_kstat->ks_data = (user_addr_t)&kmem_cache_kstat;
...
kmem_fini(void)
{
	...
	list_destroy(&kmem_deferred_slab_list);

	mutex_destroy(&kmem_cache_kstat_lock);
	mutex_destroy(&kmem_cache_lock);
	mutex_destroy(&kmem_deferred_slab_lock);
...
kmem_init(void)
{
	...
	mutex_init(&kmem_cache_kstat_lock, NULL, MUTEX_DEFAULT, NULL);
	mutex_init(&kmem_deferred_slab_lock, NULL, MUTEX_DEFAULT, NULL);

	list_create(&kmem_caches, sizeof (kmem_cache_t),
	    offsetof(kmem_cache_t, cache_link));

	list_create(&kmem_deferred_slab_list, sizeof (deferred_slab_t),
	    offsetof(deferred_slab_t, slab_link));

	kmem_msb_arena = vmem_create("kmem_msb", NULL, 0, PAGESIZE,
	    segkmem_alloc, segkmem_free, kmem_metadata_arena, 0,
	...
	kmem_default_arena = vmem_create("kmem_default", NULL, 0, PAGESIZE,
	    segkmem_alloc, segkmem_free, kmem_va_arena, 0,
	    VMC_DUMPSAFE | VM_SLEEP);
