...
#ifdef __DARWIN__
		if (tqe->tqent_thread == NULL) {
			printf("taskq_bucket_dispatch: entry %p not ready\n", tqe);
...
taskq_member(taskq_t *tq, kthread_t *thread)
{
	...
	for (i = 0; i < tq->tq_nthreads_max; ++i) {
...
#ifdef __DARWIN__
	t = thread_create(NULL, 0, taskq_thread, tq, 0, &p0, TS_RUN,  tq->tq_pri);
#else
	if (tq->tq_proc != &p0) {
    ...
    int minalloc, int maxalloc, struct proc *proc, uint_t dc, uint_t flags)
{
...
#ifdef __DARWIN__
	if ((nthreads > 10) && (flags & TASKQ_PREPOPULATE)) {
	...
	if (flags & TASKQ_DYNAMIC) {
		if ((tq->tq_kstat = kstat_create("unix", instance,
			...
			tq->tq_kstat->ks_data = (user_addr_t)&taskq_d_kstat;
		...
		if ((tq->tq_kstat = kstat_create("unix", instance, tq->tq_name,
			...
			tq->tq_kstat->ks_data = (user_addr_t)&taskq_kstat;
...
#ifdef __DARWIN__
	ASSERT(b != (taskq_bucket_t *)0xbaddcafe);
